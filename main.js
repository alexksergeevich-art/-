const VER = "dragon-evo-solid-stream-build-20260308h";
...
function triggerBgmDuck(amount = 0.08, duration = 220){
  bgmDuckAmount = Math.max(bgmDuckAmount, clamp(amount, 0, 0.22));
...
function registerSummonCombo(hero){
  ...
  triggerBgmDuck(combo >= 10 ? 0.10 : 0.08, 260);
...
function spawnBattleNotice(title, subtitle = '', tone = 'hit'){
  ...
  const existing = [...noticeLayer.querySelectorAll('.battle-notice')];
  const el = document.createElement('div');
  el.className = `battle-notice ${tone}`;
  el.style.top = `${18 + Math.min(3, existing.length) * 6}%`;
...
function enqueueHero(level, entry = {}, reason = "queue"){
  const queue = state.queueEntries.get(level) || [];
  queue.push({ displayName: String(entry.displayName || `LVL ${level} HERO`), source: String(entry.source || 'ui') });
