const VER = "dragon-evo-solid-stream-build-20260308j";
...
function hideWinnerOverlayAndArchive(){
  ...
  if (state.awaitingEvolutionAfterOverlay) startDragonEvolution();
}
function updateWinnerOverlay(now = performance.now()){
  if (state.winnerOverlayPending && !state.dragon.alive && state.dragon.animKey === 'death' && allRoundWinnersInPlace()){
    ...
    if (hero){
      ...
      status(`Чемпион ${hero.displayName || `LVL ${hero.level} HERO`} показан в центре. После карточки дракон начнёт эволюцию.`);
    }
    else {
      state.winnerOverlayPending = false;
      state.awaitingEvolutionAfterOverlay = true;
...
function handleDragonDeath(hero){
  ...
  status(`Дракон ${state.dragon.level} уровня повержен. Сначала победители займут свои места, затем появится карточка чемпиона и только после этого начнётся эволюция.`);
}
function respawnDragonStronger(){
  ...
  const statIndex = Math.min(DRAGON_STATS.hp.length - 1, state.dragon.level - 1);
  const overflow = Math.max(0, state.dragon.level - DRAGON_STATS.hp.length);
  const extraMul = overflow ? Math.pow(1.22, overflow) : 1;
  state.dragon.maxHp = Math.round(DRAGON_STATS.hp[statIndex] * extraMul);
  state.dragon.hp = state.dragon.maxHp;
  state.dragon.damage = Math.round(DRAGON_STATS.damage[statIndex] * (overflow ? Math.pow(1.14, overflow) : 1));
  state.dragon.alive = true;
  state.dragon.attackTimer = Math.max(0.56, 0.92 - (state.dragon.level - 1) * 0.035);
  ...
  status(`Дракон переродился и стал сильнее: lvl ${state.dragon.level}. Все победители возвращаются в бой, чемпион сохранил оставшееся HP и получил ауру.`);
}
...
function updateDragon(dt){
  ...
  if (state.dragon.alive){
    ...
    if (state.dragon.attackTimer <= 0){
      ...
      state.dragon.attackTimer = Math.max(0.50, 1.05 - (state.dragon.level - 1) * 0.045);
    }
  }
