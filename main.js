const VER = "dragon-evo-devpanel-20260308c";
...
const DEV_STORAGE_KEY = "dragon_dev_widget_v1";
...
const SPRITES = {
  ...
  dragon: {
    lvl1: {
      ...
      evo:    { url:`assets/dragon/dragon_sprite_evo1lvl_sprite.png?v=${VER}`, frameW:560, frameH:560, frames:12, fps:12 },
    },
    lvl2: {
...
function updateHeroAI(hero){
  ...
  if (hero.aiState === "winnerTravel" || hero.aiState === "winner"){
    ...
    if (distance(hero, winnerPoint) <= 10) hero.aiState = "winner";
    return;
  }
...
function handleDragonDeath(hero){
  ...
  hero.aiState = 'winnerTravel';
  hero.mode = 'walk';
  hero.frame = 0;
  hero.frameAcc = 0;
  spawnDamageFx(state.dragon.x, state.dragon.y - 100, 0, true, 'УБИТ');
  spawnImpactFx(state.dragon.x, state.dragon.y - 30, 'death');
  status(`Дракон ${state.dragon.level} уровня повержен. Идёт смерть, 3 секунды трупа и перерождение в lvl ${state.dragon.pendingLevel}.`);
}
function respawnDragonStronger(){
  ...
  status(`Дракон переродился и стал сильнее: lvl ${state.dragon.level}. HP ${state.dragon.maxHp}, урон ${state.dragon.damage}.`);
}
...
function resize(){
  ...
  for (const hero of state.heroes.values()){
    ...
    if (hero.aiState === 'winner' || hero.aiState === 'winnerTravel') {
      const winnerPoint = makeWinnerPoint(hero.level);
      hero.winnerTargetX = winnerPoint.x;
      hero.winnerTargetY = winnerPoint.y;
      hero.targetX = winnerPoint.x;
...
resize();
syncAllQueueBadges();
status(`LVL 1–5 подключены. Дракон lvl1/lvl2 теперь использует смерть, 3 сек удержания трупа, эволюцию и перерождение. В DEV доступны attack + winner точки для LVL 1–5.`);
