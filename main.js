...
const topLeftCard = document.getElementById("streamTopLeft");
const topLeftKicker = document.getElementById("streamTopLeftKicker");
const topLeftTitle = document.getElementById("streamTopLeftTitle");
...
const topSummonerMetricLabel = document.getElementById("topSummonerMetricLabel");
const topDragonLevel = document.getElementById("topDragonLevel");
const topDragonKillStat = document.getElementById("topDragonKillStat");
const topDragonDamageStat = document.getElementById("topDragonDamageStat");
...
function streamTopKey(entry = {}, level = 1){
  const name = String(entry?.displayName || entry?.name || `LVL ${level} HERO`).trim() || `LVL ${level} HERO`;
  const source = String(entry?.source || entry?.summonSource || 'ui').trim() || 'ui';
...
function shortMetric(value = 0){
  const n = Math.max(0, Number(value || 0));
  if (n >= 1000000) return `${(n / 1000000).toFixed(n >= 10000000 ? 0 : 1).replace(/\.0$/, '')}M`;
  if (n >= 1000) return `${(n / 1000).toFixed(n >= 10000 ? 0 : 1).replace(/\.0$/, '')}K`;
  return String(Math.round(n));
}
function topSlideDescriptors(){
  return [
    {
      key: 'summoner',
      kicker: 'ALL TIME TOP',
      title: 'Top Summoner',
      metricLabel: 'SUMMON',
      entry: state.streamTop?.allTimeTop || STREAM_TOP_DEFAULTS.allTimeTop,
      metric: (state.streamTop?.allTimeTop || STREAM_TOP_DEFAULTS.allTimeTop).summons || 0
    },
    {
      key: 'damager',
      kicker: 'ALL TIME TOP',
      title: 'Top Damager',
      metricLabel: 'DAMAGE',
      entry: state.streamTop?.topDamager || STREAM_TOP_DEFAULTS.topDamager,
      metric: (state.streamTop?.topDamager || STREAM_TOP_DEFAULTS.topDamager).damage || 0
    },
    {
      key: 'winner',
      kicker: 'ALL TIME TOP',
      title: 'Top Winner',
      metricLabel: 'WINS',
      entry: state.streamTop?.topWinner || STREAM_TOP_DEFAULTS.topWinner,
      metric: (state.streamTop?.topWinner || STREAM_TOP_DEFAULTS.topWinner).wins || 0
    }
  ];
}
function currentTopSlide(){
  const slides = topSlideDescriptors();
  const idx = ((state.streamTop?.slideIndex || 0) % slides.length + slides.length) % slides.length;
  return slides[idx];
}
function animateTopSlideSwap(){
  if (!topLeftCard) return;
  topLeftCard.classList.remove('is-slide-anim');
  void topLeftCard.offsetWidth;
  topLeftCard.classList.add('is-slide-anim');
}
function renderStreamTop(){
  const slide = currentTopSlide();
  const entry = slide.entry || {};
  if (topLeftKicker) topLeftKicker.textContent = slide.kicker;
  if (topLeftTitle) topLeftTitle.textContent = slide.title;
  if (topSummonerMetricLabel) topSummonerMetricLabel.textContent = slide.metricLabel;
  if (topSummonerAvatar) topSummonerAvatar.src = entry.avatar || DEFAULT_WINNER_AVATAR;
  if (topSummonerCount) topSummonerCount.textContent = shortMetric(slide.metric || 0);
  if (topNameOrbitText) topNameOrbitText.textContent = orbitNameLabel(entry.name || slide.title);
  if (topDragonLevel) topDragonLevel.textContent = `MAX LVL ${Math.max(1, Math.round(state.streamTop?.dragonRecordLevel || 1))}`;
  if (topDragonKillStat) topDragonKillStat.textContent = `Kill: ${shortMetric(state.streamTop?.dragonKillCount || 0)}`;
  if (topDragonDamageStat) topDragonDamageStat.textContent = `Damage: ${shortMetric(state.streamTop?.totalDragonDamage || 0)}`;
}
function rotateTopSlide(now = performance.now()){
  if (!state.streamTop) return;
  if (!state.streamTop.nextSlideAt) state.streamTop.nextSlideAt = now + 5000;
  if (now < state.streamTop.nextSlideAt) return;
  const slides = topSlideDescriptors();
  state.streamTop.slideIndex = ((state.streamTop.slideIndex || 0) + 1) % slides.length;
  state.streamTop.nextSlideAt = now + 5000;
  animateTopSlideSwap();
  renderStreamTop();
}
function ensureTopBaseEntry(entry = {}, level = 1, metricKey = 'summons'){
  return {
    name: String(entry?.displayName || entry?.name || `LVL ${level} HERO`),
...
function registerDamageForTop(entry = {}, level = 1, damage = 0){
  const amount = Math.max(0, Number(damage || 0));
  if (!amount) return;
  const key = streamTopKey(entry, level);
  const prev = state.streamTop.damageTotals.get(key) || { ...ensureTopBaseEntry(entry, level, 'damage') };
  prev.name = String(entry?.displayName || entry?.name || prev.name || `LVL ${level} HERO`);
  prev.avatar = String(entry?.avatar || prev.avatar || heroAvatarForEntry(entry, level));
  prev.damage = Math.max(0, Number(prev.damage || 0)) + amount;
  state.streamTop.damageTotals.set(key, prev);
  state.streamTop.totalDragonDamage = Math.max(0, Number(state.streamTop.totalDragonDamage || 0)) + amount;
  const currentTop = state.streamTop.topDamager || STREAM_TOP_DEFAULTS.topDamager;
  if (!currentTop || prev.damage > (currentTop.damage || 0)){
    state.streamTop.topDamager = { key, name: prev.name, avatar: prev.avatar, damage: prev.damage };
  }
  renderStreamTop();
}
function registerWinForTop(entry = {}, level = 1){
  const key = streamTopKey(entry, level);
  const prev = state.streamTop.winTotals.get(key) || { ...ensureTopBaseEntry(entry, level, 'wins') };
  prev.name = String(entry?.displayName || entry?.name || prev.name || `LVL ${level} HERO`);
  prev.avatar = String(entry?.avatar || prev.avatar || heroAvatarForEntry(entry, level));
  prev.wins = Math.max(0, Number(prev.wins || 0)) + 1;
  state.streamTop.winTotals.set(key, prev);
  const currentTop = state.streamTop.topWinner || STREAM_TOP_DEFAULTS.topWinner;
  if (!currentTop || prev.wins > (currentTop.wins || 0)){
    state.streamTop.topWinner = { key, name: prev.name, avatar: prev.avatar, wins: prev.wins };
  }
  renderStreamTop();
}
...
window.dragonArenaStream = Object.assign(window.dragonArenaStream || {}, {
  ...
  setTopDamager(data = {}){
    state.streamTop.topDamager = {
      key: String(data.key || 'external-damager'),
      name: String(data.name || data.displayName || 'Top Damager'),
      avatar: String(data.avatar || DEFAULT_WINNER_AVATAR),
      damage: Math.max(0, Number(data.damage || data.count || 0))
    };
    renderStreamTop();
  },
  setTopWinner(data = {}){
    state.streamTop.topWinner = {
      key: String(data.key || 'external-winner'),
      name: String(data.name || data.displayName || 'Top Winner'),
      avatar: String(data.avatar || DEFAULT_WINNER_AVATAR),
      wins: Math.max(0, Number(data.wins || data.count || 0))
    };
    renderStreamTop();
  },
...
const state = {
  ...
  streamTop: {
    ...
    topDamager: { ...STREAM_TOP_DEFAULTS.topDamager },
    topWinner: { ...STREAM_TOP_DEFAULTS.topWinner },
    summonTotals: new Map(),
    damageTotals: new Map(),
    winTotals: new Map(),
    dragonRecordLevel: STREAM_TOP_DEFAULTS.dragonRecordLevel,
    dragonKillCount: STREAM_TOP_DEFAULTS.dragonKillCount,
    totalDragonDamage: STREAM_TOP_DEFAULTS.totalDragonDamage,
    slideIndex: 0,
    nextSlideAt: 0
  },
  dragon: {
  ...
  if (hero.alive){
    ...
    if (dragon.alive){
      ...
      if (performance.now() >= hero.spawnHoldUntil && distToDragon <= combatRange(hero.cfg.attackRange) && hero.attackCd <= 0){
        ...
        registerDamageForTop(hero, hero.level, damage);
...
function handleDragonDeath(hero){
  ...
  state.streamTop.dragonKillCount = Math.max(0, Number(state.streamTop.dragonKillCount || 0)) + 1;
  registerWinForTop(hero, hero.level);
...
resize();
renderLastWinners();
state.streamTop.nextSlideAt = performance.now() + 5000;
renderStreamTop();
syncAllQueueBadges();
