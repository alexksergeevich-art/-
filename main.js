const VER = "dragon-evo-config-aura-queue-opt-20260308f";
...
const BGM = {
  explorer: makeAudio('assets/sound/explorer_fonk.mp3', 0.34, true),
  suspense: makeAudio('assets/sound/suspence_fonk.mp3', 0.34, true),
  combat: makeAudio('assets/sound/combat_fonk.mp3', 0.36, true),
};
const SFX = {
  heroSpawn: makePool('assets/sound/hero_spawn.mp3', 3, 0.42),
  heroHit: makePool('assets/sound/hero_hit.mp3', 5, 0.18),
  hit: makePool('assets/sound/hit.mp3', 5, 0.16),
  dragonHit: makePool('assets/sound/dragon_hit.mp3', 5, 0.22),
  critical: makePool('assets/sound/critikal_hit.mp3', 3, 0.32),
  dragonAttack: makePool('assets/sound/dragon_attack.mp3', 4, 0.26),
  dragonRoar: makePool('assets/sound/dragon_roar.mp3', 3, 0.28),
  dragonDie: makePool('assets/sound/dragon_die.mp3', 2, 0.44),
  dragonEvolve: makePool('assets/sound/dragon_evolve.mp3', 2, 0.40),
  dragonFly: makePool('assets/sound/dragon_flysound.mp3', 2, 0.22),
  dragonSpawn: makePool('assets/sound/dragon_spawn.mp3', 2, 0.38),
  powerUp: makePool('assets/sound/power_up.mp3', 2, 0.34),
  victory: makePool('assets/sound/victory.mp3', 2, 0.36),
  combo2: makePool('assets/sound/combo_2.mp3', 2, 0.26),
  combo3: makePool('assets/sound/combo_3.mp3', 2, 0.26),
  combo5: makePool('assets/sound/combo_5.mp3', 2, 0.30),
  combo10: makePool('assets/sound/combo_10.mp3', 2, 0.34),
  combo20: makePool('assets/sound/combo_20.mp3', 2, 0.38),
};
...
function setMusicState(state, immediate = false){
  ...
  if (immediate){
    next.volume = Number(next.datasetBaseVolume || next.volume);
    next.currentTime = 0;
    next.play().catch(() => {});
  } else {
    next.volume = 0.01;
    next.currentTime = 0;
    next.play().catch(() => {});
    fadeTo(next, Number(next.datasetBaseVolume || 0.34), 220);
  }
...
function makeHero(level){
  ...
  return {
    ...
    maxHp: heroCfg.maxHp,
    alive: true,
    spawningUntil: performance.now() + 720,
    spawnHoldUntil: performance.now() + 1280,
    displayName: `LVL ${level} HERO`,
    attackCd: 0,
    spriteKey: "idleFront",
    frame: 0,
    frameAcc: 0,
  ...
  if (hero.alive){
    ...
    if (dragon.alive){
      ...
      if (performance.now() >= hero.spawnHoldUntil && distToDragon <= combatRange(hero.cfg.attackRange) && hero.attackCd <= 0){
        ...
        spawnBattleNotice(hero.displayName, `${critical ? 'CRITICAL HIT' : 'hit a Dragon'} -${Math.round(damage)}HP`, critical ? 'critical' : 'hit');
  ...
  if (state.dragon.alive){
    ...
    if (state.dragon.attackTimer <= 0){
      ...
      if (target && Math.hypot(target.x - combatCenter.x, target.y - combatCenter.y) < combatRange(118)){
        ...
        spawnBattleNotice(target.displayName, `took Dragon hit -${Math.round(blockedDamage)}HP`, 'danger');
