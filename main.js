const VER = "dragon-queue-lvl5-red-20260308b";
...
function setSelectedEditorItem(key){
  ...
  state.dev.selectedType = key.split("-")[0] === "ring" ? "spawn" : key.split("-")[0] === "attack" ? "attack" : "dragon";
...
function renderEditorHandles(){
  ...
  for (const item of editorItems()){
    ...
    el.style.left = `${pos.x * state.width}px`;
    el.style.top = `${pos.y * state.height}px`;
...
function buildDevWidget(){
  ...
  devWidgetSections.append(
    ...
    makeDevSection("Attack slots", "Пять боевых точек вокруг дракона.", attackItems),
  );
  refreshDevPanelState();
  syncDevExportBox();
}
