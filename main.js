...
function handleDragonDeath(hero){
  ...
  state.dragon.nextPhaseAt = performance.now() + ((Math.max(1, deathMeta.frames - 1) / deathMeta.fps) * 1000) + WINNER_OVERLAY_MS;
  ...
  status(`Дракон ${state.dragon.level} уровня повержен. Новые герои поставлены на паузу, победитель показан в центре, затем уйдёт в LAST WINNERS.`);
}
