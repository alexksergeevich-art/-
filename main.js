const VER = "dragon-evo-config-aura-queue-opt-20260308e";
...
const SFX_FILES = {
  heroSpawn: "assets/sound/hero_spawn.mp3",
  heroHit: "assets/sound/hero_hit.mp3",
  hit: "assets/sound/hit.mp3",
  criticalHit: "assets/sound/critikal_hit.mp3",
  dragonAttack: "assets/sound/dragon_attack.mp3",
  dragonDie: "assets/sound/dragon_die.mp3",
  dragonEvolve: "assets/sound/dragon_evolve.mp3",
  dragonFly: "assets/sound/dragon_flysound.mp3",
  dragonHit: "assets/sound/dragon_hit.mp3",
  dragonRoar: "assets/sound/dragon_roar.mp3",
  dragonSpawn: "assets/sound/dragon_spawn.mp3",
  powerUp: "assets/sound/power_up.mp3",
  victory: "assets/sound/victory.mp3",
  combo2: "assets/sound/combo_2.mp3",
  combo3: "assets/sound/combo_3.mp3",
  combo5: "assets/sound/combo_5.mp3",
  combo10: "assets/sound/combo_10.mp3",
  combo20: "assets/sound/combo_20.mp3",
};
const MUSIC_FILES = {
  explorer: "assets/sound/explorer_fonk.mp3",
  suspense: "assets/sound/suspence_fonk.mp3",
  combat: "assets/sound/combat_fonk.mp3",
};
const SOUND_VOLUMES = {
  heroSpawn: 0.50,
  heroHit: 0.40,
...
function makeAudio(path, { loop = false, volume = 1 } = {}){
  const audio = new Audio(`${path}?v=${VER}`);
  audio.preload = "auto";
  audio.loop = loop;
  audio.volume = volume;
  audio.load();
...
function makePool(path, size, volume){
  return Array.from({ length:size }, () => makeAudio(path, { volume }));
}
const musicTracks = {
  explorer: makeAudio(MUSIC_FILES.explorer, { loop:true, volume:SOUND_VOLUMES.explorer }),
  suspense: makeAudio(MUSIC_FILES.suspense, { loop:true, volume:SOUND_VOLUMES.suspense }),
  combat: makeAudio(MUSIC_FILES.combat, { loop:true, volume:SOUND_VOLUMES.combat }),
};
const sfx = {
  heroSpawn: makeAudio(SFX_FILES.heroSpawn, { volume:SOUND_VOLUMES.heroSpawn }),
  criticalHit: makeAudio(SFX_FILES.criticalHit, { volume:SOUND_VOLUMES.criticalHit }),
  dragonAttack: makeAudio(SFX_FILES.dragonAttack, { volume:SOUND_VOLUMES.dragonAttack }),
  dragonDie: makeAudio(SFX_FILES.dragonDie, { volume:SOUND_VOLUMES.dragonDie }),
  dragonEvolve: makeAudio(SFX_FILES.dragonEvolve, { volume:SOUND_VOLUMES.dragonEvolve }),
  dragonFly: makeAudio(SFX_FILES.dragonFly, { loop:true, volume:SOUND_VOLUMES.dragonFly }),
  dragonRoar: makeAudio(SFX_FILES.dragonRoar, { volume:SOUND_VOLUMES.dragonRoar }),
  dragonSpawn: makeAudio(SFX_FILES.dragonSpawn, { volume:SOUND_VOLUMES.dragonSpawn }),
  powerUp: makeAudio(SFX_FILES.powerUp, { volume:SOUND_VOLUMES.powerUp }),
  victory: makeAudio(SFX_FILES.victory, { volume:SOUND_VOLUMES.victory }),
  combo2: makeAudio(SFX_FILES.combo2, { volume:SOUND_VOLUMES.combo2 }),
  combo3: makeAudio(SFX_FILES.combo3, { volume:SOUND_VOLUMES.combo3 }),
  combo5: makeAudio(SFX_FILES.combo5, { volume:SOUND_VOLUMES.combo5 }),
  combo10: makeAudio(SFX_FILES.combo10, { volume:SOUND_VOLUMES.combo10 }),
  combo20: makeAudio(SFX_FILES.combo20, { volume:SOUND_VOLUMES.combo20 }),
};
const sfxPools = {
  heroHit: makePool(SFX_FILES.heroHit, 4, SOUND_VOLUMES.heroHit),
  hit: makePool(SFX_FILES.hit, 4, SOUND_VOLUMES.hit),
  dragonHit: makePool(SFX_FILES.dragonHit, 4, SOUND_VOLUMES.dragonHit),
};
const soundBank = [
  ...Object.values(musicTracks),
  ...Object.values(sfx),
...
let comboWindowUntil = 0;
let comboThresholdPlayed = 0;
...
function playQuick(audio, { reset = true, volumeScale = 1 } = {}){
  if (!audio) return;
  try{
    if (reset) audio.currentTime = 0;
    audio.volume = clamp((audio._baseVolume || audio.volume || 1) * volumeScale, 0, 1);
    audio.play().catch(() => {});
  }catch{}
}
function playFromPool(key, { volumeScale = 1, reset = true } = {}){
  const pool = sfxPools[key];
  if (!pool?.length) return;
  const idx = poolCursor.get(key) || 0;
  const audio = pool[idx % pool.length];
  poolCursor.set(key, (idx + 1) % pool.length);
  playQuick(audio, { reset, volumeScale });
}
for (const audio of soundBank) audio._baseVolume = audio.volume;
function playNamedSfx(key, opts = {}){
  const now = performance.now();
  if (opts.cooldownMs){
    const nextAllowed = soundCooldowns.get(key) || 0;
    if (now < nextAllowed) return;
    soundCooldowns.set(key, now + opts.cooldownMs);
  }
  if (sfxPools[key]) return playFromPool(key, opts);
  playQuick(sfx[key], opts);
}
function stopNamedSfx(key){
  const audio = sfx[key];
  if (!audio) return;
  try{ audio.pause(); audio.currentTime = 0; }catch{}
}
function duckMusic(durationMs = 900, factor = 0.46){
  musicDuckUntil = Math.max(musicDuckUntil, performance.now() + durationMs);
  musicDuckFactor = Math.min(musicDuckFactor, factor);
}
...
function updateMusic(dt){
  restoreMusicDuck();
  const step = clamp(dt * 1.9, 0.015, 0.09);
  for (const [key, track] of Object.entries(musicTracks)){
    const target = key === musicState ? (track._baseVolume || 0.25) * musicDuckFactor : 0;
    track.volume = lerp(track.volume, target, step);
    if (target <= 0.001 && track.volume <= 0.01){
      if (!track.paused) track.pause();
      track.currentTime = track.currentTime % Math.max(track.duration || 1, 1);
      track.volume = 0;
    } else if (audioUnlocked && track.paused){
      try{ track.play().catch(() => {}); }catch{}
    }
  }
}
function detectMusicState(){
  if (state.dragon.animKey === "death" || state.dragon.animKey === "evo") return "explorer";
  const aliveHeroes = [...state.heroes.values()].filter((hero) => hero.alive);
  if (!aliveHeroes.length) return "explorer";
  const movingToDragon = aliveHeroes.some((hero) => ["route", "engage", "championReturn"].includes(hero.aiState) && hero.mode === "walk");
  if (movingToDragon) return "suspense";
  const fighting = aliveHeroes.some((hero) => hero.mode === "attack" && !["winner", "winnerTravel", "championWinner", "championWinnerTravel"].includes(hero.aiState));
  if (fighting) return "combat";
  return "explorer";
}
function registerComboPulse(){
  const now = performance.now();
  if (now > comboWindowUntil){
    comboChainCount = 0;
    comboThresholdPlayed = 0;
  }
  comboChainCount += 1;
  comboWindowUntil = now + 2400;
  let threshold = 0;
  let sfxKey = "";
  let text = "";
  if (comboChainCount >= 20){ threshold = 20; sfxKey = "combo20"; text = "WORLD BREAKER x20"; }
  else if (comboChainCount >= 10){ threshold = 10; sfxKey = "combo10"; text = "GODLIKE x10"; }
  else if (comboChainCount >= 5){ threshold = 5; sfxKey = "combo5"; text = "LEGENDARY x5"; }
  else if (comboChainCount >= 3){ threshold = 3; sfxKey = "combo3"; text = "TRIPLE STRIKE x3"; }
  else if (comboChainCount >= 2){ threshold = 2; sfxKey = "combo2"; text = "DOUBLE POWER x2"; }
  if (threshold && threshold > comboThresholdPlayed){
    comboThresholdPlayed = threshold;
    playNamedSfx(sfxKey, { cooldownMs: 250 });
    duckMusic(1400, 0.34);
    status(`${text}. Серия призывов продолжается...`);
  }
}
...
function dragonIsBusyForMusic(){ return dragonInTransition() || state.dragon.animLocked; }
...
function spawnHeroFromQueue(level){
  ...
  playSynth("spawn");
  playNamedSfx("heroSpawn", { cooldownMs:120 });
  duckMusic(380, 0.72);
  selectLevel(level);
  ...
  if (hero.alive){
    ...
    if (dragon.alive){
      ...
      if (distToDragon <= combatRange(hero.cfg.attackRange) && hero.attackCd <= 0){
        ...
        dragon.hitFx = 0.18;
        const isCriticalAudio = hero.championAura || hero.level >= 5 || Math.random() < 0.12;
        playSynth("slash");
        playNamedSfx("hit", { volumeScale:0.96 });
        playNamedSfx("heroHit", { volumeScale:isCriticalAudio ? 1.08 : 0.94 });
        playNamedSfx("dragonHit", { volumeScale:isCriticalAudio ? 1.08 : 0.96 });
        if (isCriticalAudio) playNamedSfx("criticalHit", { cooldownMs:140 });
        duckMusic(180, 0.78);
        spawnDamageFx(dragon.x, dragon.y - 66, damage, true, isCriticalAudio ? `CRIT -${Math.round(damage)}` : "");
...
function applyChampionAura(hero){
  ...
  playNamedSfx("powerUp", { cooldownMs:300 });
  duckMusic(1200, 0.40);
...
function handleDragonDeath(hero){
  ...
  playNamedSfx("dragonDie", { cooldownMs:500 });
  playNamedSfx("victory", { cooldownMs:800 });
  stopNamedSfx("dragonFly");
  duckMusic(4500, 0.16);
...
function respawnDragonStronger(){
  ...
  stopNamedSfx("dragonFly");
  playNamedSfx("dragonSpawn", { cooldownMs:500 });
  playNamedSfx("dragonRoar", { cooldownMs:500 });
  duckMusic(2200, 0.34);
...
function updateDragon(dt){
  ...
  if (!state.dragon.alive){
    if (state.dragon.animKey === "death" && state.dragon.nextPhaseAt && now >= state.dragon.nextPhaseAt){
      ...
      playNamedSfx("dragonEvolve", { cooldownMs:800 });
      playNamedSfx("dragonFly", { cooldownMs:800, reset:true });
      duckMusic(3200, 0.24);
  ...
  if (state.dragon.alive){
    ...
    if (state.dragon.attackTimer <= 0){
      ...
      if (target && Math.hypot(target.x - combatCenter.x, target.y - combatCenter.y) < combatRange(118)){
        ...
        playNamedSfx("dragonAttack", { cooldownMs:120 });
        playNamedSfx("dragonRoar", { cooldownMs:260 });
        duckMusic(240, 0.62);
...
function onSpawnButton(level){
  ...
  const queueAndNotify = (reason = "queue") => {
    state.queueCounts.set(level, (state.queueCounts.get(level) || 0) + 1);
    syncQueueBadge(level);
    spawnRingGlow(level);
    const queuedNow = state.queueCounts.get(level);
    if (reason === "dragon"){
      status(`LVL ${level} поставлен в очередь до респавна дракона. В очереди: x${queuedNow}.`);
    } else {
      status(`LVL ${level} добавлен в очередь. Ожидают: x${queuedNow}.`);
    }
  };

  if (dragonInTransition()){
    queueAndNotify("dragon");
    return;
  }
...
function tick(now){
  ...
  setMusicState(detectMusicState());
  updateMusic(dt);
...
resize();
syncAllQueueBadges();
status(`LVL 1–5 подключены. Точки загружены из stage-config, очередь активна, а все фонк/боевые звуки уже встроены в цикл игры.`);
